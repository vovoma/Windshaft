...
// @param ttl time to live in second for localized resources
// @param mml_store the MMLStore instance
  ...
  purger: function() {
    if ( ! LocalizedResourcePurger.running ) {
      ...
      this.mml_store.purgeLocalizedResources(this.ttl, function(err) {
          if ( err ) {
            console.log("Error purging localized resources: " + err);
          }
...
module.exports = function(opts){
    ...
    var redisPool = new RedisPool(opts.redis);
    ...
    app.sendError = function(res, err, statusCode, label, tolog) {
      ...
      //if ( tolog.stack ) console.log(tolog.stack); 
    ...
    */
    ...
    app.get(app.base_url + '/style', function(req, res){
        ...
        Step(
            ...
            function(err, data){
                ...
                if ( req.profiler ) req.profiler.sendStats();
            }
        );
    });
    ...
    app.createLayergroup = function(cfg, req, callback) {
        ...
        if ( req.profiler ) req.profiler.done('createLayergroup.enter');

        Step(
            function initLayergroup(){
                if ( req.profiler ) req.profiler.done('createLayergroup.Step');
                var next = this;
                ...
                for ( var i=0; i<cfg.layers.length; ++i ) {
                  ...
                  if ( lyropt.attributes ) attrLayers.push(i);
                }

                if ( req.profiler ) req.profiler.done('initLayergroup.layercheck');

                map_store.save(mapConfig, function(err, id, known) {
                  mapID = id;
                  if ( req.profiler ) req.profiler.done('initLayergroup.map.save');
                  if (err) { next(err); return; }
                ...
                var next = function(err) {
                  if (! err) finish();
                    ...
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig " + mapID + " on tile fetching error: " + e2);
                      finish(err);
                    });
                  } 
                }
                app.tryFetchTileOrGrid(req, response.layergroupid, testX, testY, testZ, 'png', undefined, next);
            },
            function tryFetchGrid(err){
                ...
                var next = function(err) {
                  if ( err ) {
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig " + mapID + " on grid fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
                  ...
                  app.tryFetchTileOrGrid(req, response.layergroupid, testX, testY, testZ, 'grid.json', layerId, next);
                }
                next();
            },
            function tryFetchTorque(err){
                ...
                var next = function(err) {
                  if ( err ) {
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig " + mapID + " on torque tile fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
                  ...
                  app.tryFetchTileOrGrid(req, response.layergroupid, testX, testY, testZ, 'json.torque', layerId, next);
                }
                next();
            },
            function tryFetchAttributes(err){
                ...
                var next = function(err) {
                  if ( err ) {
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig " + mapID + " on attributes tile fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
                  ...
                  app.tryFetchFeatureAttributes(req, response.layergroupid, layerId, next);
                }
                next();
            },
            function fetchTorqueMetadata(err){
                ...
                var next = function(err, meta) {
                  if ( err || ( next_layer && ! meta ) ) {
                    ...
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig " + mapID + " on torque metadata fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
